...
use base_roxy_traits::{Cache, CacheError};
use bytes::Bytes;
...
mod tests {
    ...
    fn test_immutable_policies(#[case] method: &str, #[case] _is_immutable: bool) {
        ...
        assert!(
            ...
            "Expected {method} to have Immutable policy"
        );
    }
        ...
        match cache.get_policy(method) {
            CachePolicy::Ttl(duration) => {
                assert_eq!(
                    duration.as_secs(),
                    expected_secs,
                    "Expected {method} to have TTL of {expected_secs} seconds"
                );
            }
            _ => panic!("Expected {method} to have TTL policy"),
        }
    }
    ...
    fn test_no_cache_policies(#[case] method: &str) {
        ...
        assert!(
            ...
            "Expected {method} to have NoCache policy"
        );
    }
    ...
    fn test_unknown_method_uses_default(#[case] method: &str) {
        ...
        assert!(
            matches!(cache.get_policy(method), CachePolicy::NoCache),
            "Expected unknown method {method} to use default NoCache policy"
        );
    }
