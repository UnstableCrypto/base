...
use base_roxy_cache::MemoryCache;
use base_roxy_config::RoxyConfig;
use base_roxy_rpc::RpcCodec;
use base_roxy_server::{ServerBuilder, create_router};
...
impl AppBuilder {
    ...
    pub async fn build(self, config: &RoxyConfig) -> Result<base_roxy_server::Router> {
...
///
/// Returns an error if the application cannot be built.
pub async fn build_app(config: &RoxyConfig) -> Result<base_roxy_server::Router> {
