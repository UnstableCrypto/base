...
use base_roxy_rpc::{JsonRpcError, ParsedResponse, ParsedResponsePacket, RpcCodec};
    ...
    pub fn to_json_rpc_error(&self) -> JsonRpcError {
        match self {
            ...
            Self::MethodBlocked(method) => {
                JsonRpcError::new(-32601, format!("method '{method}' is not allowed"))
            }
...
mod tests {
    ...
    fn test_error_display(#[case] err: ServerError, #[case] expected: &str) {
        ...
        assert!(display.contains(expected), "Expected '{display}' to contain '{expected}'");
    }
