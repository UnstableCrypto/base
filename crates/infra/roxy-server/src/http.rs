...
use base_roxy_backend::{BackendGroup, BackendResponse};
use base_roxy_rpc::{
...
use base_roxy_traits::{Cache, RateLimitResult, RateLimiter};
...
pub struct HttpAppState<C: Cache = base_roxy_cache::MemoryCache> {
...
///
/// ```ignore
/// use base_roxy_server::{create_router, ServerBuilder};
///
...
fn create_request_packet(request: &ParsedRequest) -> Result<RequestPacket, serde_json::Error> {
    ...
    let params = request.params.clone().unwrap_or_else(|| {
        RawValue::from_string("[]".to_string()).expect("empty array is valid JSON")
    });
...
mod tests {
    ...
    use base_roxy_traits::DefaultCodecConfig;
    ...
    fn test_app_state_debug() {
        let state: HttpAppState<base_roxy_cache::MemoryCache> = HttpAppState::new(
