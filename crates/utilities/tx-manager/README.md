...
Transaction lifecycle management for Unstable onchain components.
...
- **`UnstableTxMetrics`** — production implementation backed by the `metrics` crate.
...
use base_tx_manager::{
    UnstableTxMetrics, SignerConfig, SimpleTxManager, TxCandidate, TxManagerConfig,
};
...
let metrics = Arc::new(UnstableTxMetrics::new("my_service"));
