...
//!
//! Unstable Azul introduces a per-transaction gas limit cap of 2^24 (16,777,216).
...
fn transfer_request_without_gas() -> UnstableTransactionRequest {
    UnstableTransactionRequest::default().from(Account::Alice.address()).to(Account::Bob.address())
}

fn transfer_request_with_data_without_gas() -> UnstableTransactionRequest {
    transfer_request_without_gas().input(TransactionInput::new(Bytes::from_static(&[0x00])))
}

fn transfer_request_with_gas_and_data() -> UnstableTransactionRequest {
    transfer_request_with_data_without_gas().gas_limit(100_000)
}

#[tokio::test]
async fn azul_gas_limit_cap() -> Result<()> {
    let chain_spec = Arc::new(UnstableChainSpec::from_genesis(build_test_genesis_azul()));
...
async fn pre_azul_accepts_tx_above_gas_limit_cap() -> Result<()> {
    let chain_spec = Arc::new(UnstableChainSpec::from_genesis(build_test_genesis()));
    let harness = TestHarnessBuilder::new().with_chain_spec(chain_spec).build().await?;
...
async fn azul_eth_call_with_data_to_contract_accepts_implicit_gas_limit() -> Result<()> {
    ...
    let request = UnstableTransactionRequest::default()
