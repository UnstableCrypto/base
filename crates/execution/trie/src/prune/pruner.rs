...
use crate::{
    UnstableProofsStorage, UnstableProofsStore,
    prune::{
        error::{UnstableProofStoragePrunerResult, PrunerError, PrunerOutput},
        metrics::Metrics,
    },
};
...
#[derive(Debug)]
pub struct UnstableProofStoragePruner<P, H> {
...
impl<P, H> UnstableProofStoragePruner<P, H> {
    /// Create a new pruner.
    pub const fn new(
        provider: UnstableProofsStorage<P>,
        block_hash_reader: H,
...
where
    ...
    H: BlockHashReader,
{
    fn run_inner(&self) -> UnstableProofStoragePrunerResult {
...
mod tests {
    ...
    async fn run_inner_and_and_verify_updated_state() {
        ...
        let store: UnstableProofsStorage<Arc<MdbxProofsStorage>> =
            UnstableProofsStorage::from(Arc::new(MdbxProofsStorage::new(dir.path()).expect("env")));
        ...
        let pruner = UnstableProofStoragePruner::new(store.clone(), block_hash_reader, 1, 1000);
    ...
    async fn run_inner_where_latest_block_is_none() {
        ...
        let pruner = UnstableProofStoragePruner::new(store, block_hash_reader, 10, 1000);
    ...
    async fn run_inner_earliest_none_real_db() {
        ...
        let block_hash_reader = MockBlockHashReader::new();
        let pruner = UnstableProofStoragePruner::new(store, block_hash_reader, 1, 1000);
        let out = pruner.run_inner().expect("ok");
        assert_eq!(out, PrunerOutput::default(), "should early-return default output");
    }
    ...
    async fn run_inner_interval_too_small_real_db() {
        ...
        let block_hash_reader = MockBlockHashReader::new();
        let pruner = UnstableProofStoragePruner::new(store, block_hash_reader, 2, 1000);
        let out = pruner.run_inner().expect("ok");
