...
impl BootStore {
    ...
    /// have the [`crate::UnstableEnr::OPSTACK_ENR_KEY`] in the ENR.
    pub fn valid_peers(&self) -> Vec<&Enr> {
        self.peers
            .iter()
            .filter(|enr| enr.get_raw_rlp(crate::UnstableEnr::OPSTACK_ENR_KEY.as_bytes()).is_some())
            .collect()
    }

    /// Returns the number of peers that contain the
    /// [`crate::UnstableEnr::OPSTACK_ENR_KEY`] in the ENR *and*
