...
impl EnrValidation {
    ...
    pub fn validate(enr: &Enr, chain_id: u64) -> Self {
        let base_enr = match UnstableEnr::try_from(enr) {
...
/// The error type that can be returned when trying to convert an [`Enr`] to a [`UnstableEnr`].
#[derive(Debug, thiserror::Error, Clone, PartialEq, Eq)]
pub enum UnstableEnrError {
    /// Missing Unstable ENR key.
    #[error("Missing Unstable ENR key")]
    MissingKey,
    /// Failed to decode the Unstable ENR Value.
    #[error("Failed to decode the Unstable ENR Value: {0}")]
    DecodeError(String),
    /// Invalid version.
...
impl TryFrom<&Enr> for UnstableEnr {
    type Error = UnstableEnrError;
    fn try_from(enr: &Enr) -> Result<Self, Self::Error> {
        let Some(mut opstack) = enr.get_raw_rlp(Self::OPSTACK_ENR_KEY) else {
            return Err(UnstableEnrError::MissingKey);
        };
        let base_enr =
            Self::decode(&mut opstack).map_err(|e| UnstableEnrError::DecodeError(e.to_string()))?;

        if base_enr.version != 0 {
            return Err(UnstableEnrError::InvalidVersion(base_enr.version));
        }
...
impl Encodable for UnstableEnr {
...
impl Decodable for UnstableEnr {
...
mod tests {
    ...
    fn roundtrip_base_enr() {
        arbtest::arbtest(|u| {
            let base_enr = UnstableEnr::from_chain_id(u.arbitrary()?);
            let bytes = alloy_rlp::encode(base_enr);
            let decoded = UnstableEnr::decode(&mut &bytes[..]).unwrap();
    ...
    fn test_enr_validation() {
        ...
        let base_enr = UnstableEnr::from_chain_id(8453);
        let mut base_enr_bytes = Vec::new();
        base_enr.encode(&mut base_enr_bytes);
        enr.insert_raw_rlp(UnstableEnr::OPSTACK_ENR_KEY, base_enr_bytes.into(), &key).unwrap();
    ...
    fn test_enr_validation_invalid_version() {
        let key = CombinedKey::generate_secp256k1();
        let mut enr = Enr::builder().build(&key).unwrap();
        let mut base_enr = UnstableEnr::from_chain_id(8453);
        base_enr.version = 1;
        let mut base_enr_bytes = Vec::new();
        base_enr.encode(&mut base_enr_bytes);
        enr.insert_raw_rlp(UnstableEnr::OPSTACK_ENR_KEY, base_enr_bytes.into(), &key).unwrap();
