...
pub trait L2SourceClient: Debug + Send + Sync {
    ...
    /// Fetches a block by number and converts it to an [`UnstableExecutionPayloadEnvelope`].
    async fn get_payload_by_number(
        &self,
        number: u64,
    ) -> Result<UnstableExecutionPayloadEnvelope, DelegateL2ClientError>;
}

/// Client that polls a source L2 execution layer node for block data and
/// converts blocks into [`UnstableExecutionPayloadEnvelope`] for engine insertion.
#[derive(Debug, Clone)]
...
impl DelegateL2Client {
    ...
    pub fn new(url: Url) -> Self {
        let provider = RootProvider::<Unstable>::new_http(url);
        Self { provider }
    }
}
...
impl L2SourceClient for DelegateL2Client {
    ...
    async fn get_payload_by_number(
        ...
        let txs: Vec<UnstableTxEnvelope> = rpc_block
            .transactions
            .into_transactions()
            .map(|tx| tx.inner.inner.into_inner())
            .collect();

        let consensus_block: Block<UnstableTxEnvelope> = Block {
        ...
        let (execution_payload, _sidecar) =
            UnstableExecutionPayload::from_block_unchecked(block_hash, &consensus_block);

        Ok(UnstableExecutionPayloadEnvelope { parent_beacon_block_root, execution_payload })
    }
}
