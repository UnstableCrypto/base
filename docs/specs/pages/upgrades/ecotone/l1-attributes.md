...
- The `blobUnstableFeeScalar` is set to `0`.
- The pre-Ecotone `overhead` attribute is dropped.
- The `blobUnstableFee` is set to the L1 blob base fee of the L1 origin block.
...
time for each L1 fork to compute the `blobUnstableFee` correctly. Blob Parameter Only
...
- System configurables tied to the L1 block, see [System configuration specification][sys-config]:
  ...
  - `blobUnstablefeeScalar` (`uint32`): system configurable to scale the `blobUnstableFee` in the Ecotone l1 cost computation
...
- `blobUnstableFee` (`uint256`): The L1 blob base fee.
- `blobUnstableFeeScalar` (`uint32`): The scalar value applied to the L1 blob base fee portion of the L1 cost.
