...
During sync, we expect the withdrawals list in the block body to be empty (Unstable does not make
...
As it stands, the `withdrawalsRoot` field is unused within Unstable's header consensus format, and will never be
used for other reasons that are currently planned. Setting this value to the account storage root of the withdrawal
directly fits with Unstable, and makes use of the existing field in the L1 header consensus format.

#### Client Implementation Considerations

Various EL clients store historical state of accounts differently. If, as a contrived case, Unstable did not have
...
`DEPOSIT_REQUEST_TYPE`. Deposit requests then appear in the [EIP-7685] requests list. Unstable needs to ignore these
...
In Unstable, `EIP-7685` is no-op'd, and the `requestsHash` is always set to `sha256('')` (as noted in
...
enabled either. The Unstable execution layer must ensure that the post-block filtering of events in the deposit contract
(EIP-6110) as well as the `EIP-7002` + `EIP-7251` system calls are _not invoked_ during the block sealing process after
Isthmus activation.

Users of Unstable may still permissionlessly deploy these smart contracts, but they will not be treated as special
by the Unstable execution layer, and the system calls introduced in L1's Pectra hardfork are not considered.
...
The operator fee is integrated directly into the EVM, alongside the standard gas fee and the Unstable-specific L1 data
...
`blobUnstableFeeScalar` used in the [`L1Fee`](../../protocol/execution/index.md#ecotone-l1-cost-fee-changes-eip-4844-da).
